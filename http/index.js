...
const convertEdgeEventToGateway = require('./lib/edgeToGateway');
// const createEdgeResponse = require('./lib/createEdgeResponse');
...
class API {
  ...
  constructor(props) {
    ...
    this._provider = props && props.provider;
    // Set sampling info
    this._sampleCounts = {}
  ...
  async run(event,context,cb) {
    ...
    if(this._provider === 'awsEdge'){
      this._event = convertEdgeEventToGateway(this._event.Records[0].cf);
    }
    ...
    // if(this._provider === 'awsEdge'){
    //     caches[request.params.id] = response._response);
    // }
