...
const createEdgeResponse = require('./createEdgeResponse');
...
class RESPONSE {
  ...
  constructor(app, request) {
    ...
    this._helmet = {
      dnsPrefetchControl : (headers, options) => {
    ...
    this._helmetHtmlOnly = {

      // https://github.com/helmetjs/x-xss-protection
      xssFilter : (headers, options) => {
  ...
  // Adds a header express style
  setHeader(key, value, append) {
    return this.header(key, value, append);
  }
  ...
  httpSecurityHeaders(opts = {}) {
    opts = Object.assign({}, this._defaultSecurityHeaders, opts)
    this._headers = this._headers || {}

    Object.keys(this._helmet).forEach(key => {
      const options = Object.assign({}, this._defaultSecurityHeaders[key], opts[key])
      this._headers = this._helmet[key](this._headers, options)
    })

    if (this._headers['Content-Type'] && this._headers['Content-Type'].indexOf('text/html') !== -1) {
      Object.keys(this._helmetHtmlOnly).forEach(key => {
        const options = Object.assign({}, this._defaultSecurityHeaders[key], opts[key])
        this._headers = this._helmetHtmlOnly[key](this._headers, options)
      })
    }
  ...
  send(body) {
    ...
    if (this.app._provider === 'awsEdge') {
      this._response = createEdgeResponse(this._response);
    }
